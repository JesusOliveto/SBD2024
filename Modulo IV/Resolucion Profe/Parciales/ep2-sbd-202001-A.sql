...
(
 ...
 check (nro_cliente != garante),
 constraint CK__creditos__interes_diario__END 
...
-- 1. Agregar la columna nro_recibo a la tabla cuotas para asociar las cuotas a los recibos
alter table cuotas
add nro_recibo integer null;
go

-- 3. Establecer la columna nro_recibo en cuotas como una clave externa que referencia a recibos
alter table cuotas
add constraint fk_cuotas_recibos
foreign key (nro_recibo) references recibos (nro_recibo);
go

-- 2. Actualizar la tabla cuotas con los datos de la tabla det_recibos
update cuotas
set nro_recibo = dr.nro_recibo
from cuotas c
inner join det_recibos dr
on c.nro_credito = dr.nro_credito and c.nro_cuota = dr.nro_cuota;
go


-- 4. Eliminar la tabla det_recibos ya que no es necesaria con los nuevos requerimientos
drop table det_recibos;
go
...
SELECT 
    ...
    SUM(cu.importe) AS saldo_vencido_cuotas
FROM 
    creditos c
JOIN 
    cuotas cu ON c.nro_credito = cu.nro_credito
WHERE 
    cu.fecha_vto < GETDATE() -- Cuotas con fecha de vencimiento pasada
    AND cu.nro_recibo IS NULL -- Cuotas que no tienen recibo asociado (no pagadas)
GROUP BY 
    c.nro_credito
HAVING 
    COUNT(*) > 0 -- Asegura que solo se incluyan créditos con cuotas vencidas


select c.nro_credito, count(*), sum(cu.importe) as saldo_vencido_cuotas from creditos c join cuotas cu on c.nro_credito = cu.nro_credito where cu.fecha_vto < getdate() and cu.nro_recibo is null group by c.nro_credito 
go

select * from cuotas where nro_recibo is null