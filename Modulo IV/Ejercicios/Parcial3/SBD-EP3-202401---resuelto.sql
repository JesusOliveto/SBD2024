...
--profe
select e.cod_carrera, e.cod_materia, e.año_cursado, e.nro_alumno
  from examenes e
       join carreras c
	     on e.cod_carrera = c.cod_carrera
 where e.nota_examen < c.nota_aprobacion
 group by e.cod_carrera, e.cod_materia, e.año_cursado, e.nro_alumno, c.cant_max_aplazos--el group by es necesario pq sino te va a mostrar todos los registror de un alumno en una materia/carrera/año
...
--yo
select e.cod_carrera,e.cod_materia,e.nro_alumno,e.año_cursado,c.cant_max_aplazos, count(e.nota_examen) as aplazos
	from examenes e
		join carreras c 
			on e.cod_carrera = c.cod_carrera
    where e.nota_examen < c.nota_aprobacion
    group by e.cod_carrera,e.cod_materia,e.nro_alumno,e.año_cursado,c.cant_max_aplazos
...
-- 3. programación de triggers
create or alter trigger tiu_ri_examenes
on examenes
for insert, update
as
begin
   if exists (select e.cod_carrera, e.cod_materia, e.año_cursado, e.nro_alumno--obten estas columnas
                from examenes e
...
begin
   ...
   if exists (select e.cod_carrera, e.cod_materia, e.año_cursado, e.nro_alumno --(5)
               ...
               where -- esto es para ser más específico en el análisis (2)
			         (
			          i.cant_max_aplazos < d.cant_max_aplazos -- si baja la cantidad máxima de aplazos
					  or
					  i.nota_aprobacion  > d.nota_aprobacion  -- si sube la nota de aprobación mínima
					 )
               ...
               group by e.cod_carrera, e.cod_materia, e.año_cursado, e.nro_alumno --(3)
...
as
begin
   select a.nro_alumno, a.nom_alumno, cu.año_cursado
     from cursados cu
	      join alumnos a
    ...
    where cu.regular = 'S' --trae todos los alumnos regulares
	  and ca.cant_max_aplazos > (select count(*)
								  ...
								  where cu.cod_carrera = e.cod_carrera
									...
									and cu.año_cursado = e.año_cursado
      ...
      and cu.año_cursado + ca.años_regularidad >= year(@fecha_examen) -- verifica que el año que consulta sea menor a la regularidad del alumno,esto te va a mostrar años anteriores a la regularidad
	 ...
	 and cu.año_cursado<=year(@fecha_examen)
end
go
